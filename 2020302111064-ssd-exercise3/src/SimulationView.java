...
public class SimulationView {
    public static String getHtml(Simulation simulation) 
    {
        ...
        htmlPage.setTitle("Move Catfish Right");//设置网页标题
        htmlPage.addText("<H2> Move Catfish Right</H2>");//添加网页文本
        HtmlImage htmlImage1 = new HtmlImage("/right-arrow", "right");//the image here is right-arrow.gif
        String str = getImageButton("Submit", "right1", htmlImage1);
        HtmlTable htmlTable1 = new HtmlTable();
        htmlTable1.startRow();
        htmlTable1.addCell(str);
        htmlTable1.endRow();//创建表单
        HtmlTable htmlTable2 = new HtmlTable(simulation.getMaxColumn());
        HtmlImage htmlImage2 = new HtmlImage("/blank", "No fish.");//the image here is blank.gif
        for (byte b = 1; b <= simulation.getMaxRow(); b++) 
        {
            htmlTable2.startRow();
            for (byte b1 = 1; b1 <= simulation.getMaxColumn(); b1++) //b为行,b1为列
            {
                String str1 = "aqua";
